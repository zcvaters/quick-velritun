...
func TypingTest(request events.APIGatewayProxyRequest) (*events.APIGatewayProxyResponse, error) {
	...
	if testRequest.Words > 20 {
		return apiResponse(http.StatusBadRequest, TypingTestResponse{Error: fmt.Sprint("Invalid request words must be less then 20.")}, err)
	}

	req, err := http.NewRequest("GET", wordsURL, nil)
	if err != nil {
		return apiResponse(http.StatusBadRequest, TypingTestResponse{Error: fmt.Sprintf("Failed to create request: %v", err)}, err)
	}

	res, err := http.DefaultClient.Do(req)
	if err != nil {
		return apiResponse(http.StatusBadRequest, TypingTestResponse{Error: fmt.Sprintf("Failed to do request: %v", err)}, err)
	}
	...
	if err != nil {
		return apiResponse(http.StatusBadRequest, TypingTestResponse{Error: fmt.Sprintf("Failed to read all body: %v", err)}, err)
	}

	var testResponse MonkeyTypeResponse
	err = json.Unmarshal(body, &testResponse)
	if err != nil {
		return apiResponse(http.StatusBadRequest, TypingTestResponse{Error: fmt.Sprintf("Failed to unmarshal data: %v", err)}, err)
	}
	...
	for i := 0; i < testRequest.Words; i++ {
		...
		if err != nil {
			return apiResponse(http.StatusInternalServerError, TypingTestResponse{Error: fmt.Sprintf("Failed to generate int: %v", err)}, err)
		}
